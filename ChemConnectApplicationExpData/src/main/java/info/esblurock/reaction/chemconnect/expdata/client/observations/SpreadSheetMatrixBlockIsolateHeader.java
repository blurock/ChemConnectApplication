...
import info.esblurock.reaction.chemconnect.core.base.client.modal.ChooseFromHierarchyNode;
...
public class SpreadSheetMatrixBlockIsolateHeader extends Composite 
	...
	public void conceptChosen(String topconcept, String concept, ArrayList<String> path) {
		if(startrowB) {
			...
			async.getCatalogObject(concept,MetaDataKeywords.observationsFromSpreadSheetFull,
					new AsyncCallback<DatabaseObjectHierarchy>() {

						@Override
						public void onFailure(Throwable caught) {
							StandardWindowVisualization.errorWindowMessage("Original Matrix", caught.toString());
						}

						@Override
						public void onSuccess(DatabaseObjectHierarchy result) {
							setInHierarchy(result);
						}
	...
	private void applyMatrixIsolation(String basename) {
		...
		async.isolateFromMatrix(isolatedcatid, observationsFromSpreadSheet, spread,
				new AsyncCallback<DatabaseObjectHierarchy>() {

					@Override
					public void onFailure(Throwable caught) {
						StandardWindowVisualization.errorWindowMessage("Application", caught.toString());
					}
