...
public class IsolateBlockFromMatrix {
	...
	public static DatabaseObjectHierarchy isolateFromMatrix(DataCatalogID catid, DatabaseObjectHierarchy matrixhier,
		...
		boolean title = titleS.compareTo(ExpDataMetaDataKeywords.matrixBlockTitleFirstLine) == 0;
		boolean firstcolumn = titleS.compareTo(ExpDataMetaDataKeywords.matrixBlockTitlesFirstColumnTranspose) == 0;
		 ...
		 */
		DatabaseObjectHierarchy infohier = InterpretExpDataData.SpreadSheetInputInformation.createEmptyObject(catid);
		SpreadSheetInputInformation newinput = (SpreadSheetInputInformation) infohier.getObject();
		newinput.setSourceType(ExpDataStandardDataKeywords.chemConnectDataObject);
		newinput.setSource(matrix.getIdentifier());
		newinput.setType(ExpDataMetaDataKeywords.matrixBlockIsolated);
		newinput.setDelimitor("none");

		DatabaseObjectHierarchy newmatrixhier = null;
		if(firstcolumn) {
			newmatrixhier = CreateSpreadSheetObjects.fillObservationsFromSpreadSheet(catid,
	...
	private static int determineEndColumn(int begincolumn, ObservationValueRow row,
		...
		if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockColumnEndMaximum) == 0) {
			endcolumn = row.getRow().size() - 1;
		} else if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockColumnEndAtPosition) == 0) {
			try {
				endcolumn = Integer.valueOf(endinfo).intValue();
			} catch (NumberFormatException ex) {
				throw new IOException("Column begin position not a number: '" + endinfo + "'");
			}
		} else if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockColumnEndNumberOfColumns) == 0) {
			try {
	...
	private static int determineBeginColumn(ObservationValueRow row, SpreadSheetBlockIsolation blockisolate)
		...
		if (begintype.compareTo(ExpDataMetaDataKeywords.matrixBlockColumnBeginLeft) == 0) {
			begincolumn = 0;
		} else if (begintype.compareTo(ExpDataMetaDataKeywords.matrixBlockColumnBeginAtPosition) == 0) {
			try {
				begincolumn = Integer.valueOf(begininfo).intValue();
			} catch (NumberFormatException ex) {
				throw new IOException("Column begin position not a number: '" + begininfo + "'");
			}
		} else if (begintype.compareTo(ExpDataMetaDataKeywords.matrixBlockColumnBeginAtIdentifier) == 0) {
			if (begininfo == null) {
	...
	private static int determineEndRow(int start, DatabaseObjectHierarchy multhier,
		...
		int endrow = start;
		if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockEndAtBlankLine) == 0) {
			try {
				endrow = searchForBlankLine(start, multhier.getSubobjects());
			} catch (IOException ex) {
				throw new IOException("Blank Line Expected for rows");
			}
		} else if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockEndAtFileEnd) == 0) {
			endrow = multhier.getSubobjects().size() - 1;
		} else if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockEndAtFileEndOrBlankLine) == 0) {
			try {
				endrow = searchForBlankLine(start, multhier.getSubobjects());
			} catch (IOException ex) {
				endrow = multhier.getSubobjects().size() - 1;
			}
		} else if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockEndAtIdentifierExclusive) == 0) {
			endrow = findIdentifierInRows(0, endinfo, multhier, false) - 1;
		} else if (endtype.compareTo(ExpDataMetaDataKeywords.matrixBlockEndAtIdentifierInclusive) == 0) {
	...
	private static int determineBeginRow(DatabaseObjectHierarchy multhier, SpreadSheetBlockIsolation blockisolate)
		...
		int beginrow = 0;
		if (begintype.compareTo(ExpDataMetaDataKeywords.beginMatrixAfterSpecifiedIdentifier) == 0) {
			beginrow = findIdentifierInRows(0, begininfo, multhier, false);
		} else if (begintype.compareTo(ExpDataMetaDataKeywords.beginMatrixAtSpecified) == 0) {
			beginrow = Integer.valueOf(begininfo).intValue();
		} else if (begintype.compareTo(ExpDataMetaDataKeywords.beginMatrixTopOfSpreadSheet) == 0) {
			beginrow = 0;
		} else if (begintype.compareTo(ExpDataMetaDataKeywords.beginMatrixAtStartsWithSpecifiedIdentifier) == 0) {
